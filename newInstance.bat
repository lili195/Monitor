...
docker run -d -p %PUERTO_MAPEADO% --name %CONTENEDOR% -e DATABASE_URL -e ID_SERVICE=server%CONTADOR% -e NODE_SERVICE_IP=172.17.0.%CONTADOR% -e NODE_SERVICE_PORT=%PORT% -e NODE_VERSION=18.19.1 -e PATH=%DOCKERFILE_DIR% -e YARN_VERSION=1.22.19 %IMAGEN%
